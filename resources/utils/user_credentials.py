...
class Serial:
    ...
    def macaddr(self):
        ...
        another_way = ':'.join(['{:02x}'.format((uuid.getnode() >> i) & 0xff) for i in range(0, 8*6, 8)][::-1])
    ...
    def motherboardSerialNumber(self):
        try:
            if system == "Mac":
                ...
                return serial_number[0] if serial_number else "Unknown-Motherboard-Serial"
            
            elif system == "Windows":
                return subprocess.check_output(["wmic", "baseboard", "get", "serialnumber"],
    ...
    def storageSerialNumber(self):
        ...
        try:
            if system == "Windows":
                return subprocess.check_output(["wmic",  "diskdrive",  "get",  "serialnumber"],
                                               creationflags=subprocess.CREATE_NO_WINDOW,  # type: ignore
                                               ).decode().split('\n')[1].strip()
            elif system == "Mac":
